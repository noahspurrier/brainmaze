...
void Build_Maze (Maze m)
{
    ...
    else
    {
        ...
        Combine_Submazes_Into_Maze (m,
            maze_quarter_1, maze_quarter_2, maze_quarter_3, maze_quarter_4);
...
void Combine_Submazes_Into_Maze (Maze m, Maze mq1, Maze mq2, Maze mq3, Maze mq4)
{
    ...
    do
        x = random (sub_size);
    until (Maze_Get(m, x, sub_size-1) == BLOCK  &&
           Maze_Get(m, x, sub_size+1) == BLOCK);

    Maze_Put (m, x, sub_size, BLOCK);

    do
        y = random (sub_size);
    until (Maze_Get(m, sub_size-1, y) == BLOCK  &&
           Maze_Get(m, sub_size+1, y) == BLOCK);
